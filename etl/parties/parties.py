...
def get_wiki_link(party_name):
    ...
    "engine": "google",
    "q": query,
    "api_key": "73df8f2974a001ef1a4440094b1c98c7d2b6c8f04cdefddbb2a76c89bbe27a4e"
    }
...
def get_party_info(wiki_link):
    if not wiki_link:
        return {"summary": None, "position": None, "error": "No link found"}
    ...
    return {"summary": summary, "position": position}
...
def insert_into_table(party_dict):
    ...
    required_keys = ["name", "position", "summary"]
    ...
    try:
        ...
        insert_query = sql.SQL("""
            INSERT INTO candidate_data.parties (name, position, summary)
            VALUES (%s, %s, %s)
            RETURNING id;
        """)
        ...
        cur.execute(
            insert_query,
            (party_dict["name"], party_dict["position"], party_dict["summary"])
        )
...
def main():
    ...
    for party in parties:
        ...
        party_info = get_party_info(link) if link else {"summary": "not found", "position": "not found"}
        if isinstance(party_info, dict):
            party.update(party_info)
            
        else:
            # fallback if something went wrong
            party.update({"summary": "not found", "position": "not found"})
